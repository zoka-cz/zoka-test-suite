...
namespace Zoka.TestSuite.BasicTestActions
{
	internal class ForAction : IPlaylistAction
	{
		...
		{
			var start_val = (int)ZScriptExpressionParser.ParseScriptExpression(m_StartValueExpr).EvaluateExpressionToValue(_data_storages, _service_provider);
			_data_storages.Store(m_IndexerExpr, start_val);

			while ((bool)ZScriptExpressionParser.ParseScriptExpression(m_ConditionExpr).EvaluateExpressionToValue(_data_storages, _service_provider))
			{
				...
				var expr_res = (int)ZScriptExpressionParser.ParseScriptExpression(m_ActionExpr).EvaluateExpressionToValue(_data_storages, _service_provider);
		...
		public static ForAction								ParseFromXmlElement(FileInfo _src_file, XElement _x_element, List<IFunctionAction> _imported_functions, IServiceProvider _service_provider)
		{
			...
			var action_expr = _x_element.ReadAttr<string>("action", _src_file, true);
